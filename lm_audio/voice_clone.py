# NOTE: There is an open issue on the OrpheusTTS repo about voice cloning
# see https://github.com/canopyai/Orpheus-TTS/issues/6
...
assert hf_token, "missing hugginface access token"
login(token=hf_token)

MODEL = 'canopylabs/3b-es_it-pretrain-research_release' #'canopylabs/3b-es_it-ft-research_release'

def load_orpheus_tokenizer(model_id) -> AutoTokenizer:
...
def load_orpheus_auto_model(model_id):
...
if __name__ == "__main__":
    tokenizer = load_orpheus_tokenizer(MODEL)
    model = load_orpheus_auto_model(MODEL)
    snac_model = load_snac()

    texts = [
        "Il ragazzo senza vergogna"
    ]

    path_to_audio = "data/audio/M01UD01p10.wav"
    transcript = "Ora che hai fatto il pieno di informazioni utili, prenditi un momento per pensarci su e rispondi a questa domanda."
